...
export default function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const discount = comparePrice ? getDiscountPercentage(comparePrice, currentPrice) : 0;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
              ...
              {[1, 2, 3, 4].map((i) => (
                <button
                  ...
                  className={cn(
                    ...
                    i === 1 ? "border-gold" : "border-transparent hover:border-neutral-200"
                  )}
                >
          ...
          >
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs text-neutral-300">SKU: {product.sku}</span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
              {[
                ...
                { icon: RefreshCw, label: "15-Day Returns", sub: "Easy refund" },
              ].map(({ icon: Icon, label, sub }) => (
                <div key={label} className="flex flex-col items-center text-center p-3 rounded-xl bg-neutral-50">
                  <Icon className="w-4 h-4 text-gold-dark mb-1" />
                  <span className="text-[10px] font-medium text-obsidian">{label}</span>
            ...
            >
              ...
              {activeTab === "specs" && (
                <div className="space-y-3">
                  {[
                    ...
                    <div key={spec.label} className="flex items-center py-2 border-b border-neutral-100">
                      <span className="text-sm text-neutral-400 w-32">{spec.label}</span>
                      <span className="text-sm text-obsidian">{spec.value}</span>
                    </div>
                  ))}
                </div>
              )}
              {activeTab === "shipping" && (
                <div className="text-sm text-neutral-600 space-y-4 leading-relaxed">
                  <p><strong>Free Shipping:</strong> We offer free shipping on all orders above ₹999 across India.</p>
                  <p><strong>Delivery Time:</strong> Standard delivery takes 5-7 business days. Express delivery (2-3 days) is available at checkout.</p>
                  <p><strong>Returns:</strong> We offer a 15-day hassle-free return policy. Items must be in original condition with all tags attached.</p>
                  <p><strong>Exchanges:</strong> Lifetime exchange available on all gold jewelry.</p>
                </div>
              )}
              {activeTab === "care" && (
                <div className="text-sm text-neutral-600 space-y-4 leading-relaxed">
                  <p><strong>Storage:</strong> Store your jewelry in the box provided, away from moisture and direct sunlight.</p>
                  <p><strong>Cleaning:</strong> Gently clean with a soft cloth. For gold jewelry, use warm soapy water and a soft brush.</p>
                  <p><strong>Avoid:</strong> Remove jewelry before swimming, bathing, or using chemicals.</p>
                  <p><strong>Professional Cleaning:</strong> We recommend professional cleaning once a year for diamond jewelry.</p>
                </div>
              )}
            </motion.div>
          </AnimatePresence>

...
export default function AboutPage() {
  return (
    <div className="min-h-screen">
      {/* Hero */}
      <section className="relative py-24 md:py-32 bg-gradient-to-b from-soft-pink-light to-ivory overflow-hidden">
        <div className="absolute top-10 right-10 w-60 h-60 bg-rose-gold/5 rounded-full blur-3xl" />
        <div className="container-luxury text-center relative">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <p className="text-xs tracking-[0.3em] uppercase text-rose-gold-dark mb-4">Our Story</p>
            <h1 className="font-heading text-4xl md:text-6xl text-deep-plum mb-6">
              Crafting Beauty,<br />
              One Piece at a Time
            </h1>
        ...
        <div className="container-luxury">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              >
                <div className="inline-flex items-center justify-center w-14 h-14 bg-rose-gold/10 rounded-2xl mb-4">
                  <value.icon className="w-6 h-6 text-rose-gold-dark" />
                </div>
                <h3 className="font-heading text-lg text-deep-plum mb-2">{value.title}</h3>
      ...
      <section className="py-20">
        <div className="container-luxury text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="font-heading text-3xl text-deep-plum mb-4">Ready to Discover?</h2>
            <p className="text-neutral-400 mb-8">Explore our latest collection and find the piece that speaks to you.</p>
            <a href="/collections" className="inline-flex items-center gap-2 bg-deep-plum text-white px-8 py-4 rounded-full text-sm font-medium hover:bg-neutral-800 transition-colors btn-glow">
              Shop the Collection
            </a>
          </motion.div>
        </div>
